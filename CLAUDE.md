...
- Public pages must remain reviewable, render meaningful visible content, and stay crawlable
- Monetized pages must not be blocked by `robots.txt` or conflicting meta robots directives
- Placeholder, incomplete, or non-launch-ready pages must remain `draft` and `noindex`
- Trust and legal pages must exist and remain accessible: `/about`, `/contact`, `/privacy-policy`, `/terms`
- Layouts should preserve comfortable whitespace and stable content flow so future ad placements can be added without pushing core content below the fold or degrading UX
- Any future `ads.txt` data should be served cleanly from the root-level `/ads.txt` route and populated from configuration rather than hardcoded ad snippets in templates

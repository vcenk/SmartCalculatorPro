...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const category = await loadCategory('everyday-life');
...
export default async function EverydayLifeCategoryPage() {
  const category = await loadCategory('everyday-life');
  const siteConfig = await loadSiteConfig();
  ...
  const allCalculators = await getCalculatorsByCategory('everyday-life');
  const calculators = allCalculators.filter(
    (calc) => calc.status === 'published' && hasCalculatorFunction(calc.id)
  );

  const breadcrumbs = generateBreadcrumbsFromPath('/everyday-life', siteConfig.url);
    ...
    <div className="flex flex-col">
      ...
      <Section>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
          <CategoryBreadcrumbs category="everyday-life" />
          ...
          {calculators.length > 0 ? (
            <>
              <h2 className="text-2xl font-semibold mb-4">Everyday Life Calculators</h2>
            ...
            <div className="mb-8 rounded-2xl border border-[#dce7f1] bg-white/80 p-6 shadow-sm">
              ...
              <p className="text-muted-foreground mb-4">
                This category is still being expanded. For now, browse the live finance calculators while the everyday-life toolset grows.
              </p>

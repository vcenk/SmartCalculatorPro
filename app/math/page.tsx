...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const category = await loadCategory('math');
...
export default async function MathCategoryPage() {
  const category = await loadCategory('math');
  const siteConfig = await loadSiteConfig();
  ...
  const allCalculators = await getCalculatorsByCategory('math');
  const calculators = allCalculators.filter(
    (calc) => calc.status === 'published' && hasCalculatorFunction(calc.id)
  );

  const breadcrumbs = generateBreadcrumbsFromPath('/math', siteConfig.url);
    ...
    <div className="flex flex-col">
      ...
      <Section>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
          <CategoryBreadcrumbs category="math" />
          ...
          {calculators.length > 0 ? (
            <>
              <h2 className="text-2xl font-semibold mb-4">Math Calculators</h2>
            ...
            <div className="mb-8 rounded-2xl border border-[#dce7f1] bg-white/80 p-6 shadow-sm">
              ...
              <p className="text-muted-foreground mb-4">
                This category is still being expanded. For now, browse the live finance calculators while the math toolset grows.
              </p>
